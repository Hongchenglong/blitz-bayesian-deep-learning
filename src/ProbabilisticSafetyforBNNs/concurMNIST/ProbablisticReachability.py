    ...
    for i in range(len(vector_intervals[0])):
        for j in range(len(vector_intervals[0][0])):
            ...
            for num_found in range(len(vector_intervals)):
                interval = [vector_intervals[num_found][i][j] - (stds[i][j] * marg),
                            vector_intervals[num_found][i][j] + (stds[i][j] * marg)]
...
def compute_interval_probs_bias(vector_intervals, marg, mean, std):
    ...
    for i in range(len(vector_intervals[0])):
        ...
        for num_found in range(len(vector_intervals)):
            ...
            interval = [vector_intervals[num_found][i] - (stds[i] * marg),
                        vector_intervals[num_found][i] + (std[i] * marg)]
            intervals.append(interval)
...
def linear_propogation_VCAS(x, in_reg, out_ind,
    ...
    for i in trange(search_samps):
        ...
        if (np.argmax(y) == out_ind or extra_gate):
            ...
            [y_pred_l, y_pred_u] = propogate_lines(x, in_reg, [[sW_0[i]], [sW_1[i]]], [[sb_0[i]], [sb_1[i]]], w_margin,
                                                   act)

            y_pred_l = np.squeeze(y_pred_l);
            y_pred_u = np.squeeze(y_pred_u)
            print(y_pred_l)
            print(y_pred_u)
            print(y_pred_l <= y)
            print(y_pred_u >= y)
