...
class network(object):
    ...
    def deeppoly(self):
        ...
        for i in range(len(self.layers) - 1):
            ...
            if cur_layer.layer_type == layer.AFFINE_LAYER:  # affine layer, Wx + b
                for j in range(cur_layer.size):
                    ...
                    pre(cur_neuron, i)
            elif cur_layer.layer_type == layer.RELU_LAYER:  # relu layer, relu(Wx + b)
                for j in range(cur_layer.size):
                    cur_neuron = cur_neuron_list[j]
                    ...
                    if pre_neuron.concrete_highest_lower >= 0 or cur_neuron.certain_flag == 1:
                        cur_neuron.algebra_lower = np.zeros(cur_layer.size + 1)  # 加1个b
                ...
                if (line.startswith('#')):
                    ...
                    if (linedata[4] == 'Input'):
                        ...
                        for i in range(layersize[-1]):
                            ...
                            new_layer.neurons.append(new_neuron)
                        self.layers.append(new_layer)  # add affine layer
                        dicts.append(layerdict)
                        # add relu layer
                        new_layer = layer()
                        new_layer.layer_type = layer.RELU_LAYER
